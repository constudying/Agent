...
class Config(dict):
    ...
    def __init__(__self, *args, **kwargs):
        object.__setattr__(__self, '__key_locked', False) # 控制是否允许添加新的key，防止意外添加未定义的参数
        object.__setattr__(__self, '__all_locked', False) # 控制是否允许堆配置对象的任何修改（包括键和值）
        object.__setattr__(__self, '__do_not_lock_keys', False) # 控制是否允许对健进行锁定操作，由于某些配置对象可能需要始终允许添加新key（动态扩展对象）
        object.__setattr__(__self, '__parent', kwargs.pop('__parent', None)) # 保存父配置对象的引用（在嵌套配置结构中，便于追溯或操作父级配置）
        object.__setattr__(__self, '__key', kwargs.pop('__key', None)) # 保存当前配置对象在父对象中的键名（用于嵌套结构中，定位在父配置中的位置）
        for arg in args:
            if not arg:
                continue
