...
class GPT_wrapper(nn.Module):
    ...
    def __init__(self, feat_dim, n_layer, n_head, block_size, gmm_modes, action_dim, proprio_dim,
                 spatial_softmax_num_kp, gmm_min_std, dropout, crop_height, crop_width, device):
        super().__init__()
    ...
    def random_crop_grid(self, x, grid):
        delta = x.size(2) - grid.size(1)
        grid = grid.repeat(x.size(0), 1, 1, 1).to(self.device)
        # Add random shifts by x
        grid[:, :, :, 0] = grid[:, :, :, 0] + torch.FloatTensor(x.size(0)).to(self.device).random_(0, delta).unsqueeze(
            -1).unsqueeze(-1).expand(-1, grid.size(1), grid.size(2)) / x.size(2)
        # Add random shifts by y
        grid[:, :, :, 1] = grid[:, :, :, 1] + torch.FloatTensor(x.size(0)).to(self.device).random_(0, delta).unsqueeze(
            -1).unsqueeze(-1).expand(-1, grid.size(1), grid.size(2)) / x.size(2)
        return grid
    ...
    def forward_step(self, obs):
        ...
        x_feature = obs['latent_plan']
        x_feature = x_feature.view(1, 1, 64).contiguous()  # NOTE: latent_plan维度
